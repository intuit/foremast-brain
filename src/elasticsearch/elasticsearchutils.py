...
payload_update_by_query = {
 ...
 "script" : {
  "inline" : "ctx._source.status='<STATUS>' ; ctx._source.modified_at='<MODIFIED_AT>'"
 }
}
...
payload_update_by_query_info = {
 ...
 "script" : {
  "inline" : "ctx._source.status='<STATUS>' ;ctx._source.info='<INFO>'; ctx._source.reason='<REASON>'; ctx._source.modified_at='<MODIFIED_AT>'"
 }
}
